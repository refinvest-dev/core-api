...
class GetBacktestResultServiceTest {
    ...
    fun `does not read a result for a pending run`() {
        val runId = BacktestRunId(10L)
        val service = GetBacktestResultService(
            backtestRunReader = runReader { pendingRun(it) },
    ...
    fun `returns null when the run does not exist`() {
        val service = GetBacktestResultService(
            backtestRunReader = runReader { null },
    ...
    private fun runReader(
        ...
        override fun findByStrategyId(
            ...
            page: Int,
            size: Int,
        ) = error("findByStrategyId is not used by GetBacktestResult")

        override fun countByMemberIdAndCreatedAtBetween(
            memberId: MemberId,
            startInclusive: Instant,
            endExclusive: Instant,
        ) = error("countByMemberIdAndCreatedAtBetween is not used by GetBacktestResult")
    }
